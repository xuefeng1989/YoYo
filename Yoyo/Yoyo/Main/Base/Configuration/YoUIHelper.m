...
@implementation YoUIHelper (QMUIMoreOperationAppearance)
+ (void)customMoreOperationAppearance {
    // 如果需要统一修改全局的 QMUIMoreOperationController 样式，在这里修改 appearance 的值即可
    [QMUIMoreOperationController appearance].cancelButtonTitleColor = UIColorTheme;
}

@end


@implementation YoUIHelper (QMUIAlertControllerAppearance)
...
@implementation YoUIHelper (QMUIDialogViewControllerAppearance)
...
+ (void)customDialogViewControllerAppearance {
    ...
    buttonTitleAttributes[NSForegroundColorAttributeName] = UIColorTheme;
    appearance.buttonTitleAttributes = [buttonTitleAttributes copy];
    appearance.buttonHighlightedBackgroundColor = UIColorClear;
}

@end


@implementation YoUIHelper (QMUIEmotionView)
...
+ (void)customEmotionViewAppearance {
    ...
    //    [QMUIEmotionView appearance].sendButtonBackgroundColor = [LGThemeManager sharedInstance].currentTheme.themeTintColor;
}

@end

@implementation YoUIHelper (QMUIImagePicker)
...
+ (void)customImagePickerAppearance {
    ...
    //    [QMUIImagePickerCollectionViewCell appearance].checkboxImage = [checkboxImage qmui_imageWithTintColor:[LGThemeManager sharedInstance].currentTheme.themeTintColor];
    //    [QMUIImagePickerCollectionViewCell appearance].checkboxCheckedImage = [checkboxCheckedImage qmui_imageWithTintColor:[LGThemeManager sharedInstance].currentTheme.themeTintColor];
    //    [QMUIImagePickerPreviewViewController appearance].toolBarTintColor = [LGThemeManager sharedInstance].currentTheme.themeTintColor;
}

@end

@implementation YoUIHelper (UITabBarItem)
...
@implementation YoUIHelper (Button)
...
+ (QMUIButton *)generateDarkFilledButton {
    ...
    //    button.backgroundColor = [LGThemeManager sharedInstance].currentTheme.themeTintColor;
    //    button.highlightedBackgroundColor = [[LGThemeManager sharedInstance].currentTheme.themeTintColor qmui_transitionToColor:UIColorBlack progress:.15];// 高亮时的背景色
...
+ (QMUIButton *)generateLightBorderedButton {
    QMUIButton *button = [[QMUIButton alloc] qmui_initWithSize:CGSizeMake(200, 40)];
    button.titleLabel.font = UIFontBoldMake(14);
    //    [button setTitleColor:[LGThemeManager sharedInstance].currentTheme.themeTintColor forState:UIControlStateNormal];
    //    button.backgroundColor = [[LGThemeManager sharedInstance].currentTheme.themeTintColor qmui_transitionToColor:UIColorWhite progress:.9];
    //    button.highlightedBackgroundColor = [[LGThemeManager sharedInstance].currentTheme.themeTintColor qmui_transitionToColor:UIColorWhite progress:.75];// 高亮时的背景色
    //    button.layer.borderColor = [button.backgroundColor qmui_transitionToColor:[LGThemeManager sharedInstance].currentTheme.themeTintColor progress:.5].CGColor;
    button.layer.borderWidth = 1;
    button.layer.cornerRadius = 4;
    //    button.highlightedBorderColor = [button.backgroundColor qmui_transitionToColor:[LGThemeManager sharedInstance].currentTheme.themeTintColor progress:.9];// 高亮时的边框颜色
    return button;
}
...
@implementation YoUIHelper (Emotion)
...
+ (void)asyncLoadImages:(NSArray<QMUIEmotion *> *)emotions {
    ...
    //            e.image = [UIImageMake(e.identifier) qmui_imageWithBlendColor:[LGThemeManager sharedInstance].currentTheme.themeTintColor];
    //        }
    //    });
}
...
@implementation YoUIHelper (SavePhoto)
...
@implementation YoUIHelper (Calculate)
...
@implementation YoUIHelper (Theme)
